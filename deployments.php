<?php
// deployments.php - Police Squad Deployments & Tactical Zone Management (Government Theme)
        ...
        <section class="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <div class="flex items-center gap-3">
                    <h2 class="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight">Police Squad Deployments</h2>
                ...
                <p class="text-xs text-slate-500 font-medium mt-1">Manage field security units, radio communication frequencies, and zone perimeters</p>
            </div>
            <button type="button" onclick="openCreateDeploymentModal()" class="px-4 py-2.5 rounded-2xl bg-[#1d63d8] hover:bg-[#1553c7] text-white text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-2 cursor-pointer">
        ...
        <div class="bg-white p-4 rounded-3xl border border-slate-200 shadow-xs">
            <form method="GET" action="deployments.php" class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center">
                ...
                <div class="sm:col-span-4">
                    <select name="mission" class="w-full py-2 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 font-bold focus:outline-none focus:border-[#1d63d8]">
                ...
                <div class="sm:col-span-2 flex items-center gap-2">
                    ...
                    <?php if (!empty($search) || !empty($filterMission)): ?>
                        <a href="deployments.php" class="p-2 bg-slate-100 hover:bg-red-50 text-slate-600 hover:text-red-600 rounded-xl border border-slate-200 transition-colors" title="Clear Filters">
                ...
                <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-xs flex flex-col justify-between space-y-4 hover:border-slate-300 hover:shadow-sm transition-all">
                    <div>
                        <div class="flex items-start justify-between gap-2 mb-3">
                            <div class="flex items-center gap-2.5">
                                <div class="w-9 h-9 rounded-xl bg-blue-50 text-[#1d63d8] border border-blue-200 flex items-center justify-center text-sm">
                                    <i class="fa-solid fa-shield-halved"></i>
                                </div>
                                <div>
                                    <h4 class="text-sm font-extrabold text-slate-900"><?= htmlspecialchars($d['unit_callsign']) ?></h4>
                                    <span class="text-[11px] text-blue-700 font-bold"><?= htmlspecialchars($d['mission_type']) ?></span>
                                </div>
                            </div>
                            <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold <?= $isActive ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-slate-100 text-slate-600 border border-slate-200' ?> mono">
                        ...
                        <div class="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1.5 text-xs text-slate-700 font-medium">
                            <p class="flex items-center gap-2">
                                <i class="fa-solid fa-location-dot text-red-500 text-xs w-4"></i>
                                <span class="font-bold text-slate-900"><?= htmlspecialchars($d['zone_name']) ?></span>
                            </p>
                            <p class="flex items-center gap-2 text-slate-600">
                                <i class="fa-solid fa-triangle-exclamation text-amber-500 text-xs w-4"></i>
                                <span><?= htmlspecialchars($d['disaster_title']) ?></span>
                            </p>
                            <p class="flex items-center gap-2 font-mono text-blue-700 font-semibold">
                                <i class="fa-solid fa-walkie-talkie text-blue-600 text-xs w-4"></i>
                    ...
                    <div class="pt-3 border-t border-slate-100 flex items-center justify-between">
                        <form method="POST" action="deployments.php" class="inline">
                            ...
                            <button type="submit" class="px-3 py-1.5 rounded-xl text-xs font-bold <?= $isActive ? 'bg-slate-100 hover:bg-slate-200 text-slate-700' : 'bg-emerald-600 hover:bg-emerald-700 text-white' ?> transition-colors cursor-pointer">
...
<!-- CREATE SQUAD MODAL -->
<div id="createDeploymentModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white border border-slate-200 rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl relative">
        ...
        <div class="flex items-center justify-between pb-4 mb-5 border-b border-slate-100">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-lg font-extrabold text-slate-900">Deploy Police Unit</h3>
                    <p class="text-xs text-slate-500 font-medium">Deploy tactical squad to crisis zone</p>
                </div>
            </div>
            <button type="button" onclick="closeCreateDeploymentModal()" class="text-slate-400 hover:text-slate-800 p-2 cursor-pointer">
        ...
        <form method="POST" action="deployments.php" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 mono">Unit Callsign *</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 mono">Officers Count *</label>
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 mono">Mission Type *</label>
                <select name="mission_type" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 font-bold focus:outline-none focus:border-[#1d63d8]">
            ...
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 mono">Zone / Sector Name *</label>
                <input type="text" name="zone_name" required placeholder="e.g. Coastline Highway 101 Access Point" 
                       class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-[#1d63d8] focus:bg-white font-medium">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 mono">Radio Frequency</label>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 mt-6">
                <button type="button" onclick="closeCreateDeploymentModal()" class="px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-colors cursor-pointer">
                    Cancel
                </button>
