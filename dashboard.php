...
<div class="flex-1 flex flex-col min-w-0 bg-[#f8fafc] h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col p-3 sm:p-4 lg:p-6 space-y-3.5 overflow-hidden">
        ...
        <!-- COMPACT TOP METRICS HUD STRIP WITH ACCENT CONTRAST -->
        <section class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 shrink-0" aria-label="Incident Statistics">
            <a href="sos.php" class="bg-white border border-slate-200 border-l-4 border-l-red-500 p-2.5 sm:p-3 rounded-2xl shadow-2xs flex items-center justify-between hover:border-slate-300 hover:shadow-xs transition-all">
                <div>
                    <p class="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider mono">Total SOS</p>
                    <h3 class="text-lg sm:text-xl font-black text-slate-900 leading-tight"><?= $totalSosCount ?></h3>
                </div>
                <div class="w-9 h-9 rounded-xl bg-red-50 border border-red-200 flex items-center justify-center text-red-600 shrink-0">
                    <i class="fa-solid fa-tower-broadcast text-xs"></i>
                </div>
            </a>

            <a href="sos.php?status=active" class="bg-white border border-slate-200 border-l-4 border-l-amber-500 p-2.5 sm:p-3 rounded-2xl shadow-2xs flex items-center justify-between hover:border-slate-300 hover:shadow-xs transition-all">
                <div>
                    <p class="text-[10px] font-extrabold text-amber-800 uppercase tracking-wider mono">Active Rescues</p>
                    <h3 class="text-lg sm:text-xl font-black text-amber-700 leading-tight"><?= $activeRescuesCount ?></h3>
                </div>
            ...
            <a href="sos.php?status=Resolved" class="bg-white border border-slate-200 border-l-4 border-l-emerald-500 p-2.5 sm:p-3 rounded-2xl shadow-2xs flex items-center justify-between hover:border-slate-300 hover:shadow-xs transition-all">
                <div>
                    <p class="text-[10px] font-extrabold text-emerald-800 uppercase tracking-wider mono">Safe / Rescued</p>
                    <h3 class="text-lg sm:text-xl font-black text-emerald-700 leading-tight"><?= $safeResolvedCount ?></h3>
                </div>
            ...
            <a href="tasks.php" class="bg-white border border-slate-200 border-l-4 border-l-[#1d63d8] p-2.5 sm:p-3 rounded-2xl shadow-2xs flex items-center justify-between hover:border-slate-300 hover:shadow-xs transition-all">
                <div>
                    <p class="text-[10px] font-extrabold text-blue-800 uppercase tracking-wider mono">Missions Open</p>
                    <h3 class="text-lg sm:text-xl font-black text-blue-700 leading-tight"><?= $openTasksCount ?></h3>
                </div>
                <div class="w-9 h-9 rounded-xl bg-blue-50 border border-blue-200 flex items-center justify-center text-[#1d63d8] shrink-0">
                    <i class="fa-solid fa-list-check text-xs"></i>
                </div>
            </a>

            <a href="deployments.php" class="bg-white border border-slate-200 border-l-4 border-l-indigo-500 p-2.5 sm:p-3 rounded-2xl shadow-2xs flex items-center justify-between hover:border-slate-300 hover:shadow-xs transition-all">
                <div>
                    <p class="text-[10px] font-extrabold text-indigo-800 uppercase tracking-wider mono">Deployed Squads</p>
                    <h3 class="text-lg sm:text-xl font-black text-indigo-700 leading-tight"><?= $activeDeploymentsCount ?></h3>
                </div>
                <div class="w-9 h-9 rounded-xl bg-indigo-50 border border-indigo-200 flex items-center justify-center text-indigo-600 shrink-0">
                    <i class="fa-solid fa-shield-halved text-xs"></i>
                </div>
            </a>

            <a href="sos.php?priority=Critical" class="bg-white border border-red-300 border-l-4 border-l-red-600 p-2.5 sm:p-3 rounded-2xl shadow-2xs flex items-center justify-between hover:border-red-400 hover:shadow-xs transition-all">
                <div>
                    <p class="text-[10px] font-extrabold text-red-700 uppercase tracking-wider mono">Critical Priority</p>
                    <h3 class="text-lg sm:text-xl font-black text-red-700 leading-tight"><?= $criticalPriorityCount ?></h3>
                </div>
                <div class="w-9 h-9 rounded-xl bg-red-100 border border-red-300 flex items-center justify-center text-red-700 animate-pulse shrink-0">
        ...
        <section id="mapWrapper" class="flex-1 w-full bg-white rounded-2xl border border-slate-200 flex flex-col relative shadow-xs overflow-hidden min-h-[480px]">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initial Tile Layer (Government Clean Streets)
    setMapTile('street');
    ...
    // Disable click propagation on custom overlay boxes
...
function setMapTile(type) {
    ...
    ['btnTileStreet', 'btnTileSat', 'btnTileDark'].forEach(btnId => {
        ...
        if (el) {
            el.className = 'px-2.5 py-1 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold text-[11px] border border-slate-200 transition-all cursor-pointer';
        }
    });
    ...
    if (activeEl) {
        activeEl.className = 'px-2.5 py-1 rounded-xl bg-[#1d63d8] text-white font-bold text-[11px] shadow-2xs transition-all cursor-pointer';
    }
}
